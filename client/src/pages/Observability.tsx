...
import { AlertCircle, CheckCircle, Info, AlertTriangle, Search, RefreshCw, Activity, Terminal, ShieldAlert, Zap } from "lucide-react";
    ...
    <DashboardLayout>
      <div className="p-3 space-y-3 bg-slate-50/50 min-h-screen">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          {[
            { label: "Total de Logs", value: logsQuery.data?.length || 0, icon: Terminal, color: "text-slate-600", bg: "bg-slate-50" },
            { label: "Erros Críticos", value: errosQuery.data?.length || 0, icon: ShieldAlert, color: "text-red-600", bg: "bg-red-50" },
            { label: "Alertas Ativos", value: alertasQuery.data?.length || 0, icon: AlertTriangle, color: "text-orange-600", bg: "bg-orange-50" },
            { label: "Uptime Global", value: "99.9%", icon: Zap, color: "text-green-600", bg: "bg-green-50" },
          ].map((stat, i) => (
                      ...
                      <div key={alerta.id} className="p-2 rounded-lg border bg-slate-50/50 hover:bg-white transition-colors group">
                        <div className="flex justify-between items-start mb-1">
                          ...
                          <Badge className={`text-[9px] h-4 px-1.5 border-0 font-bold ${
                            alerta.severidade === 'critica' ? 'bg-red-100 text-red-700' :
                            alerta.severidade === 'alta' ? 'bg-orange-100 text-orange-700' :
